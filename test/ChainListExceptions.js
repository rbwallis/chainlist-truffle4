//The tester will test exceptions thrown by the buyArticle function when
//the requirements are not fullfilled
// Basis of the test suite will be very similar to tester ChainListHappyPath

//Chainlist contract to be tested
...
// test case: if no article is for sale yet
it("should throw an exception if you try to buy an article when there is no article for sale yet", function (){
 ...
 .catch(function(error){
   ...
   // make sure the state of the contract wasn't changed by this failing function, was reverted back
 }).then(function(data){
   assert.equal(data.toNumber(), 0, "number of articles must be 0");
   });
 });

 //buy article 2 that does not exist, after selling just one article
...
// we only have one article to buy
});
 });

  //Test case: Try to buy the article that you are selling
  it("should throw an exception if you try to buy your own article", function(){
    ...
    assert.equal(data[2], 0x0, "buyer must be empty");
    assert.equal(data[3], articleName, "article name must be " + articleName);
    assert.equal(data[4], articleDescription, "article decription must be " + articleDescription);
    assert.equal(data[5].toNumber(), web3.toWei(articlePrice, "ether"), "article price must be " + web3.toWei(articlePrice, "ether"));
   });
   });

   //Test case: Whwn the price is not the same as the selling price
   it("should throw an exception if you try to buy an article for a value different from its price", function(){
     ...
     // contract will reject any value whether its lower or higher, has to be the articlePrice
   }).then(assert.fail)
     .catch(function(error){
     assert(true);
   }).then(function(){
    ...
    // Test case: try to buy article that already has been sold
    it("should throw an exception if you try to buy an article that has already been sold", function(){
      ...
      // second time when trying to buy again the same article, fails
    }).then(assert.fail)
      .catch(function(error){
      assert(true);
    }).then(function(){
